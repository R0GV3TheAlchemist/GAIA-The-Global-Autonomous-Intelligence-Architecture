...
class IdentityOperator(BaseOperator):
    ...
    def matrix(self, dim: int) -> ComplexMatrix: return np.eye(dim, dtype=np.complex128)


class PhaseOperator(BaseOperator):
    def __init__(self, theta: float) -> None: self._theta = float(theta)
    @property
    def name(self) -> str: return f"phase(theta={self._theta:.4f})"
    def matrix(self, dim: int) -> ComplexMatrix:
...
class IntentionOperator(BaseOperator):
    ...
    @property
    def name(self) -> str: return f"intention(target={self._target}, strength={self._strength:.4f})"
    def matrix(self, dim: int) -> ComplexMatrix:
        M = np.eye(dim, dtype=np.complex128)
        t = self._target % dim
        c, s = math.cos(self._strength), math.sin(self._strength)
        src = (t + 1) % dim if t == 0 else 0
        M[src, src] = c; M[src, t] = -s; M[t, src] = s; M[t, t] = c
        return M
...
class CoherenceOperator(BaseOperator):
    ...
    @property
    def name(self) -> str: return f"coherence(strength={self._strength:.2f})"
    def matrix(self, dim: int) -> ComplexMatrix:
...
class DecoherenceChannel(BaseOperator):
    ...
    @property
    def name(self) -> str: return f"decoherence(rate={self._rate:.3f})"
    def matrix(self, dim: int) -> ComplexMatrix:
...
class InterferenceChannel(BaseOperator):
    def __init__(self, src_idx: int, tgt_idx: int, strength: float = 0.2, mode: str = "constructive") -> None:
        self._src = int(src_idx); self._tgt = int(tgt_idx)
        self._strength = float(np.clip(strength, 0.0, 1.0))
