...
C27 — GAIAN Lifecycle Manager  (Phase 2)
"""
...
from .ed25519_audit import CanonicalAuditEntry, Phase2Ed25519BridgeSigner
...
class _GAIANRecord:
    __slots__ = ("gaian_id", "state", "born_at", "last_transition_at", "canonical_audit")
...
class LifecycleManager:
    def __init__(
        ...
        self._bridge_signer = Phase2Ed25519BridgeSigner()
    ...
    def _get_record(self, gaian_id: str) -> _GAIANRecord:
        ...
        if record is None:
            raise KeyError(f"[C27] GAIAN '{gaian_id}' is not registered. Call register_latent() first.")
        return record
    ...
    def _append_canonical_audit(
        ...
        entry = CanonicalAuditEntry(
            ...
            previous_entry_hash=self._bridge_signer.previous_hash(prior),
        )
        entry.signature = self._bridge_signer.sign(entry.payload_for_signing())
    ...
    def activate(self, gaian_id: str, actor_id: Optional[str] = None, justification: Optional[str] = None, trigger_class: str = "SYSTEM_EVENT") -> None:
        self._transition(gaian_id, GAIANLifecycleState.ACTIVE, LifecycleEventType.ACTIVATION, actor_id, justification=justification, trigger_class=trigger_class)
    ...
    def enter_dormancy(self, gaian_id: str, reason: str = "unspecified", actor_id: Optional[str] = None) -> None:
        self._transition(
            ...
            LifecycleEventType.DORMANCY_ENTER,
            actor_id,
    ...
    def exit_dormancy(self, gaian_id: str, actor_id: Optional[str] = None) -> None:
        self._transition(
            ...
            LifecycleEventType.DORMANCY_EXIT,
            actor_id,
    ...
    def enter_adoptable(
        ...
        self._transition(
            ...
            LifecycleEventType.ADOPTABLE_ENTER,
            actor_id,
    ...
    def assign_custodian(self, gaian_id: str, custodian_id: str, actor_id: Optional[str] = None) -> StewardshipBond:
        ...
        if record.state != GAIANLifecycleState.ADOPTABLE:
            raise ValueError(f"Custodian can only be assigned while GAIAN is ADOPTABLE (current state: {record.state.name}).")
        bond = self._stewards.assign_custodian(gaian_id, custodian_id)
        self._logger.log(gaian_id=gaian_id, event_type=LifecycleEventType.CUSTODIAN_ASSIGNED, payload={"custodian_id": custodian_id, "bond_id": bond.bond_id}, actor_id=actor_id)
        self._append_canonical_audit(
            gaian_id=gaian_id,
            event_type="STEWARD_ACTION",
            from_state=record.state.value,
    ...
    def evaluate_adoption_queue(self) -> list:
        ...
        for action in actions:
            gaian_id = action["gaian_id"]
            days = action["days"]
            self._sentinel.check_adoption_timeout(gaian_id, days)
            self._append_canonical_audit(
                gaian_id=gaian_id,
                event_type="CANON_PROCESS",
                from_state=GAIANLifecycleState.ADOPTABLE.value,
                to_state=GAIANLifecycleState.ADOPTABLE.value,
                trigger_class="CANON_PROCESS",
                actor_id="system-adoption-daemon",
                justification=f"adoption queue evaluation at day {days}",
                metadata={"queue_action": action["action"], "canon_reference": "C27"},
            )
        return actions

    def retire(self, gaian_id: str, reason: str, actor_id: Optional[str] = None) -> None:
        ...
        self._transition(
            ...
            LifecycleEventType.RETIREMENT,
            actor_id,
            extra={"reason": reason, "canon_reference": "C27"},
            justification=reason,
            trigger_class="CANON_PROCESS" if "timeout" in reason.lower() else ("STEWARD_ACTION" if actor_id else "SYSTEM_EVENT"),
        )
        self._queue.remove(gaian_id)

    def archive(self, gaian_id: str, actor_id: Optional[str] = None) -> None:
        record = self._get_record(gaian_id)
        if record.state not in _ARCHIVE_ELIGIBLE:
            ...
            raise LifecycleTransitionError(gaian_id=gaian_id, from_state=record.state, to_state=GAIANLifecycleState.ARCHIVED, reason="Only RETIRED GAIANs may be archived.")
        self._logger.log(gaian_id=gaian_id, event_type=LifecycleEventType.ARCHIVAL, payload={"from_state": record.state.value, "to_state": GAIANLifecycleState.ARCHIVED.value}, actor_id=actor_id)
        self._append_canonical_audit(
            gaian_id=gaian_id,
            event_type="CANON_PROCESS",
            from_state=record.state.value,
