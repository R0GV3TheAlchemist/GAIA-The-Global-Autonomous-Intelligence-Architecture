"""
core.gaia_runtime_manager
=========================
GAIA singleton runtime manager — owns long-lived resources
(MemoryStore, MemoryPruner, embedder) that must be shared across
all FastAPI request handlers.

Previously located at core/runtime.py, which was silently shadowed
by the core/runtime/ package directory (GAIAOrchestrator layer).
Rescued here as part of the Phase 2 alignment pass (June 2026).

Usage (in any router)
---------------------
    from core.gaia_runtime_manager import get_runtime
...
-------------------------------------
    from core.gaia_runtime_manager import GAIARuntime
    runtime = GAIARuntime()
    runtime.init()

Note on naming
--------------
This module is intentionally named gaia_runtime_manager (not runtime)
to avoid shadowing the core/runtime/ package which exports
GAIAOrchestrator — a separate, higher-level orchestration layer.
"""
...
log = logging.getLogger("gaia.runtime_manager")
