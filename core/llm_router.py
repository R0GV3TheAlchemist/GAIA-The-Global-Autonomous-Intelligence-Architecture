...
async def _call_openai(
    ...
    resp = await client.chat.completions.create(model=OPENAI_MODEL, messages=messages)
    latency = int((time.monotonic() - t0) * 1000)
...
async def stream(
    ...
    """
    Streaming token generator. Currently streams from Ollama only;
    falls back to a non-streaming cloud call chunked into words if
    Ollama is unavailable and mode allows cloud.
    """
    mode = _routing_mode()

    if mode in {RoutingMode.LOCAL_FIRST, RoutingMode.LOCAL_ONLY}:
        try:
            ...
            if mode == RoutingMode.LOCAL_ONLY:
                raise
            log.info(f"[llm_router] stream: local unavailable ({exc}), falling back to cloud (non-streaming)")

    # Cloud fallback — get full response then yield word-by-word for UX smoothness
    result = await _call_cloud(prompt, system)
